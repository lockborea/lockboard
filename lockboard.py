    ...
    for event in device.read_loop():
        ...
        if event.type == ecodes.EV_KEY:
            ...
            if event.code == ecodes.REL_WHEEL:
                ...
                if event.value > 0:
                    scroll_buttons["SCROLL_UP"] = now + duration

                elif event.value < 0:
                    scroll_buttons["SCROLL_DOWN"] = now + duration
...
while running:
    ...
    now = pygame.time.get_ticks()
    expired = [btn for btn, expiry in scroll_buttons.items() if now > expiry]
    for btn in expired:
        del scroll_buttons[btn]
    ...
    for btn, (x, y, w, h) in mouse_layout.items():
        ...
        label = font.render(
            btn.replace("BTN_", "").replace("SCROLL_", ""),
            True,
            (0, 0, 0)
        )
        label_rect = label.get_rect(center=(x + w // 2, y + h // 2))
